...
export const AdminDashboard = () => {
  const { token, showToast, logout } = useStore();
  ...
  const handleProductSubmit = async (e) => {
    ...
    try {
      ...
      const endpoint = editProductMode ? `/api/admin/products/${editProductMode}` : "/api/admin/products";
      ...
      if (res.ok) {
        ...
        showToast(err.message || "Action failed on product compilation.", "error");
      }
    } catch (e2) {
      console.error(e2);
    }
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      if (res.ok) {
        showToast(`Bulk wiped ${selectedProductIds.length} products registry.`, "success");
  ...
  const handleOrderStatusUpdate = async (orderId, status) => {
    try {
      ...
      if (res.ok) {
        showToast(`Ref tracker #${orderId} status assigned to "${status}".`, "success");
        await loadData();
      }
    } catch (e) {
      console.error(e);
  ...
  const handleUserRoleToggle = async (userId, currentRole) => {
    ...
    try {
      ...
      if (res.ok) {
        showToast("User Role toggled successfully!", "success");
        await loadData();
      } else {
        const err = await res.json();
        showToast(err.message || "Action disallowed on current user.", "error");
      }
    } catch (e) {
      console.error(e);
    }
