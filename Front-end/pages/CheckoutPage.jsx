...
export const CheckoutPage = ({ setTab, promoDiscountPrice, promoCodeApplied }) => {
  const { cart, user, token, clearCart, showToast } = useStore();
  ...
  const triggerOtpFlow = async (payMethod, payId) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setLatestGeneratedOtp(data.otp || "");
        setPendingOrderDetails({ payMethod, payId });
        setShowOtpModal(true);
        showToast("Order Confirmation OTP sent to your email!", "success");
      } else {
        const err = await res.json();
        showToast(err.message || "Failed dispatching order confirmation OTP.", "error");
      }
    } catch (e) {
  ...
  const handleCreateOrder = async (payMethod, payId, verifyingOtp) => {
    try {
      ...
      if (res.ok) {
        ...
        showToast("Order created and verified successfully!", "success");
      } else {
        const err = await res.json();
        setOtpError(err.message || "Invalid confirmation OTP.");
        showToast(err.message || "Failed creating order.", "error");
      }
    } catch (e) {
      console.error(e);
