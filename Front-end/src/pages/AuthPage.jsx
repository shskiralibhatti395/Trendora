...
export const AuthPage = ({ setTab }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (activeMode === "login") {
      ...
      if (responseStatus?.success) {
        setTab(responseStatus.user?.role === "admin" ? "admin" : "home");
      }
      ...
      try {
        ...
        showToast(err.message || "Password reset rejected.", "error");
      }
    } else if (activeMode === "verify-email") {
      showToast("Email verification is no longer required. Please sign in.", "info");
      setActiveMode("login");
    }
    setLoading(false);
  };
  const handleQuickSignIn = async (role) => {
    ...
    if (role === "admin") {
      const result = await login(mockEmail, mockPass);
      if (result?.success) setTab("admin");
    } else {
      const responseStatus = await login(mockEmail, mockPass);
      if (responseStatus?.success) setTab("home");
    }
    setLoading(false);
  };
  const isLoginGlass = activeMode === "login";
